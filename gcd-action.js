(function () {
	...
	module.exports = {
		...
		"/search/:searchfor/:day?.:page([0-9]+)?":	
		function(req, res){
			var start_time = Date.now();
			var strsearch=(typeof(req.params.searchfor)=="string"?req.params.searchfor.replace(/[\/\-\\]/g,"").substring(0,80):void(0));
			var strpage=(typeof(req.params.page)=="string"?parseInt(req.params.page):1);
			var day=(typeof(req.params.day)=="string"?parseInt(req.params.day):0);
			if (strpage > cfg.MAXPAGENUMBER) {
				res.redirect("/404");
				return;
			}
			wordseg.segment(strsearch, function(strseg){
				var countsqlstr = "SELECT topic_id FROM gcd_search WHERE rank = 1"
							+(day > 0 ?" AND updtime > DATE_SUB(CURDATE(),INTERVAL "+day+" DAY)":"")
